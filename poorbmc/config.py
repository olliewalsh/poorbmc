...
_CONFIG_FILE_PATHS = (
    os.path.join(os.path.expanduser('~'), '.pbmc', 'poorbmc.conf'),
    '/etc/poorbmc/poorbmc.conf')
...
class PoorBMCConfig(object):

    DEFAULTS = {
        'default': {
            'show_passwords': 'false',
            'config_dir': os.path.join(os.path.expanduser('~'), '.pbmc'),
        },
        'log': {
            'logfile': None,
            'debug': 'false'
